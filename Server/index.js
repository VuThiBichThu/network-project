...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('rate', async (data) => {
        console.log(data);
        const std = await Subject.findOne({ id_list_ques: data.id }, function (err, subject) {
            if (err) {
                console.log(err);
            } else if (subject) {
                console.log(subject);
                var new_numrate = subject.num_rate + 1;
                var new_rating = (subject.rating * subject.num_rate + data.rate) / new_numrate;
                console.log(new_rating + " " + new_numrate);
                Subject.where({ id_list_ques: data.id }).update({ rating: new_rating, num_rate: new_numrate }).exec();
            } else {
                console.log("Not found");
            }
        });
